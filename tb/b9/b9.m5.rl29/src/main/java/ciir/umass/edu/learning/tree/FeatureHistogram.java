...
public class FeatureHistogram {
	class Config {
		int featureIdx = -1;      //feature's idx (0-based)
		int thresholdIdx = -1;    //threshold's idx (0-based)
		double S = -1;            //score (sumL^2/cntL + sumR^2/cntR)
	}

	//Parameter
	public static float samplingRate = 1; //yyl: feature sampling rate

	//Variables
	public int[] features = null;
	public float[][] thresholds = null;
	public double[][] sum = null;         //yyl: accumulative sum for each feature's threshold
	public double sumResponse = 0;
	public double sqSumResponse = 0;
	public int[][] count = null;          //yyl: accumulative counts for each feature's threshold
	...
	{
		for(int i=start;i<=end;i++) //yy: for each feature id
		{
			int fid = features[i];			
			//System.out.println("-----Histo fid="+fid+" --------");
			...
			int last = -1;
			for(int t=0;t<threshold.length;t++) //yyl: for each uniq val in a feature
			{
				int j=last+1;
				...
				for(;j<idx.length;j++)
				{
					...
					if(samples[k].getFeatureValue(fid) > threshold[t])
						break;
					sumLeft += labels[k];
					//System.out.println("LambdaMART Histo pesudoresp="+labels[k]);//yyl
					if(i == 0)//yyl: if 1st feature, get sum_y and sumsq_y
					{
						sumResponse += labels[k];
						sqSumResponse += labels[k] * labels[k];
						//System.out.println("histo i==0 sumResp="+sumResponse);//yyl
					}//end-if
					stMap[k] =  t;
					//System.out.println("LambdaMART Histo stMap["+k+"]="+t);//yyl
				}//end-for each sample
				last = j-1;	
				sumLabel[t] = sumLeft;
				c[t] = last+1;
				//System.out.println("LambdaMART Histo samToThre="+sumLeft+" count="+c[t]);//yyl
			}
			sampleToThresholdMap[i] = stMap; //yyl: for each feature_i, map sample to threshold ID (binary_y, stMap[0]=0, stMap[8]=1)
			sum[i] = sumLabel;
			count[i] = c;       //yyl: count no. of samples < threshold
	...
	protected void update(double[] labels, int start, int end)
	{
		...
		//yyl: for each data point k, update sum_y & sumsq_y for each feature based on pseudoResp
		for(int k=0;k<labels.length;k++){
			//System.out.println("----- hist.update  k="+k+"-----"); //yyl: 

			//yyl: for each feature f
			for(int f=start;f<=end;f++){
				int t = sampleToThresholdMap[f][k];
				sum[f][t] += labels[k];     //yyl: kept track sumLeft
				//System.out.println("hist.update f="+f+" t="+t+" sum["+f+"]["+t+"]="+sum[f][t]); //yyl: 
				if(f == 0){ //yyl: cache sum_y & sumsq_y
					sumResponse += labels[k];
					sqSumResponse += labels[k]*labels[k];
				}//end-if
		...
		{			
			for(int t=1;t<thresholds[f].length;t++) {
				sum[f][t] += sum[f][t-1]; //yyl:kept track sumLeft
				//System.out.println("update f="+f+" t="+t+" sum["+f+"]["+t+"]="+sum[f][t]); //yyl: 
			}//end-for
		}//end-for
	}
	...
	protected void construct(FeatureHistogram parent, int[] soi, double[] labels, int start, int end)
	{
		...
		//yyl: cache accumulative sum & counts for each feature's threshold 
		for(int f=start;f<=end;f++){			
			for(int t=1;t<thresholds[f].length;t++){
				sum[f][t] += sum[f][t-1];
	...
	protected Config findBestSplit(int[] usedFeatures, int minLeafSupport, int start, int end)
	{
		...
		//System.out.println("Histo findBestSplit totalCount="+totalCount+" start="+start);//yyl
		for(int f=start;f<=end;f++){
			int i = usedFeatures[f]; //yyl: actual features used if sub feature sampling rate is specified
			float[] threshold = thresholds[i];

			//System.out.println("----- f="+f+" i="+i+" --------"); //yyl
			for(int t=0;t<threshold.length;t++){
				int countLeft = count[i][t];
				int countRight = totalCount - countLeft;
				//System.out.println("countL="+countLeft+" countR="+countRight+" count[i].len="+count[i].length);//yyl
				if(countLeft < minLeafSupport || countRight < minLeafSupport)
					continue;

				double sumLeft = sum[i][t];
				double sumRight = sumResponse - sumLeft;
				//System.out.println("sumL="+sumLeft+" sumR="+sumRight+" sumResp="+sumResponse);//yyl

				//yyl: S = sumL^2/cntL + sumR^2/cntR   
				double S = sumLeft * sumLeft / countLeft + sumRight * sumRight / countRight;
				//System.out.println("S="+S+" cfg.S="+cfg.S+" feaIdx="+cfg.featureIdx+" threIdx="+cfg.thresholdIdx);//yyl

				if(cfg.S < S){ //yyl: bigger score is better
					cfg.S = S;
					cfg.featureIdx = i;
					cfg.thresholdIdx = t;
				}//end-if
		...
		//System.out.println("Best cfg.S="+cfg.S+" feaIdx="+cfg.featureIdx+" threIdx="+cfg.thresholdIdx);//yyl

		return cfg;
	}
	public boolean findBestSplit(Split sp, double[] labels, int minLeafSupport)
	{
		//System.out.println("Hist findBestSplit sp.getDeviance="+sp.getDeviance()); //yyl
		if(sp.getDeviance() >= 0.0 && sp.getDeviance() <= 0.0)//equals 0
			return false;//no need to split

		//System.out.println("Hist findBestSplit samRate="+samplingRate);//yyl
		int[] usedFeatures = null;//index of the features to be used for tree splitting
		if(samplingRate < 1){//need to do sub FEATURE sampling (feature sampling)
			...
			for(int i=0;i<features.length;i++)
				usedFeatures[i] = i;
		}//end-if-else feature sub-sampling

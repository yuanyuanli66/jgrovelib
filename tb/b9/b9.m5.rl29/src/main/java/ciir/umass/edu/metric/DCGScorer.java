	...
	public DCGScorer()
	{
		...
		if(discount == null)
		{
			...
			for(int i=0;i<discount.length;i++) {
				...
				//System.out.println("DCGScorer const dist="+discount[i]); //yyl:cache discount
			}
			gain = new double[6];
			for(int i=0;i<6;i++) {
				gain[i] = (1<<i) - 1;//2^i-1
				//System.out.println("DCGScorer const gain="+gain[i]); //yyl:cache gain
	...
	public double[][] swapChange(RankList rl)
	{
		...
		for(int i=0;i<size;i++) {
			for(int j=i+1;j<rl.size();j++) {
				...
				System.out.println("swapChange changes_ji="+changes[j][i]); //yyl
			}//end-for j
		}//end-for i

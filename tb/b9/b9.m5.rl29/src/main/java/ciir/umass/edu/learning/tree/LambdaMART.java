...
public class LambdaMART extends Ranker {
	...
	public static int nTrees = 1;//the number of trees
	...
	public Ensemble ensemble = null;       //yyl: changed from protected to public
	...
	public void init()
	{
		//PRINT("Initializing LambdaMART... ");					//yyl
		System.out.println("Initializing LambdaMART... ");		//yyl
		...
		for(int i=0;i<samples.size();i++)
		{
			...
			dpCount += rl.size();
		}//end-for: for each qid (group of samples), get ranklist & data point count

		//System.out.println("LambdaMART dpCount=" + dpCount); //yyl: total # observations
		//System.out.println("LambdaMART samples.size()=" + samples.size()); //yyl: # qids

		int current = 0;
		martSamples = new DataPoint[dpCount];  //yyl: all data points/observations
		modelScores = new double[dpCount];     //yyl: model scores for all data points
		pseudoResponses = new double[dpCount];
		weights = new double[dpCount];
		for(int i=0;i<samples.size();i++) //yyl: for each datapoint, get martSample, init modelSocres, pseduoResp & weight 
		{
			RankList rl = samples.get(i);
			//System.out.println("LambdaMART rl.size()=" + rl.size()); //yyl: # sample under each qid
			for(int j=0;j<rl.size();j++)
			{
				martSamples[current+j] = rl.get(j);
				modelScores[current+j] = 0.0F;         //yyl: init to 0 for each data
				pseudoResponses[current+j] = 0.0F;     //yyl: init to 0 for each data
				weights[current+j] = 0;                //yyl: init to 0 for each data
			}//end-for each data point under given qid
			current += rl.size();
		}//end-for each qid			
		...
		MyThreadPool p = MyThreadPool.getInstance(); //yyl: get threads; if not set, get available threads
		//System.out.println("LambdaMART MyThreadPool size:"+p.size()); //yyl

		if(p.size() == 1)//single-thread
			sortSamplesByFeature(0, features.length-1); //update sortedIdx
		else//multi-thread
		{
			//System.out.println("LambdaMART multi-thread"); //yyl
		...
		for(int f=0;f<features.length;f++)
		{
			...
			for(int i=0;i<martSamples.length;i++)
			{
				...
				values.add(fv);
				//System.out.println("LambdaMART init() val=" + fv); //yyl: uniq, min & max
				if(fmax < fv)
					fmax = fv;
				if(fmin > fv)
					fmin = fv;
			...
			//System.out.println("LambdaMART nThreshold="+nThreshold); //yyl
			//yyl: if within nThre use min, max & uniq val from data, otherwise use step
			if(values.size() <= nThreshold || nThreshold == -1) //nThreshold is 256
			{
				thresholds[f] = new float[values.size()+1];
				for(int i=0;i<values.size();i++)
					thresholds[f][i] = values.get(i);
				thresholds[f][values.size()] = Float.MAX_VALUE;
				//System.out.println("LambdaMART step="+(Math.abs(fmax - fmin))/nThreshold); //yyl
			}
			else
			{
				float step = (Math.abs(fmax - fmin))/nThreshold;
				//System.out.println("LambdaMART step="+step); //yyl
	...
	public void learn()
	{
		...
		//System.out.println("LambdaMART GBM process nTrees="+nTrees); //yyl
		System.out.println("LambdaMART GBM process numSam="+samples.size()); //yyl
		...
		for(int m=0; m<nTrees; m++)
		{
			...
			rt.fit();
			//System.out.println("LambdaMart rt.toString ="+rt.toString());//yyl
			System.out.println("LambdaMart rt.toJson= " + rt.JsonToString(rt.toJson())); //yyl: print current tree
	...
	protected void computePseudoResponses(int start, int end, int current)
	{
		...
		//System.out.println("LambdaMART computePseResp start="+start+" end="+end+" current"+current);
		
		//compute the lambda for each document (a.k.a "pseudo response")
		//yyl: for each sample (multiple data-point)
		for(int i=start;i<=end;i++)
		{
			RankList orig = samples.get(i);	
			
			//for (int ii=0;ii<modelScores.length;ii++)
			//	System.out.println("ModelScore: "+modelScores[ii]); //yyl: 1 score for 1 dp
			//System.out.println("LambdaMART computePseResp current="+current + " origsz="+(current+orig.size()-1)); //yyl
			
			
			//yyl: sort data under current qid based on modelScores in descending order
			int[] idx = MergeSorter.sort(modelScores, current, current+orig.size()-1, false);
			//System.out.println("idx_len="+idx.length);
			//for (int ii=0;ii<idx.length;ii++)
			//	System.out.println("ModelScore sorted idx="+idx[ii]); //yyl: 1 score for 1 dp
			
			//yyl: compute doc_ij NDCG scores (forces/changes that moves doc up/down)
			//yyl:  - high score (bigger y value) moves to upper position (smaller idx)
			...
			for(int j=0;j<rl.size();j++)
			{
				...
				int mj = idx[j];
				//System.out.println("LambadaMART MJ="+mj); //yyl
				for(int k=0;k<rl.size();k++) //yyl
				{
					//System.out.println("LambadaMART j="+j+" k="+k);
					//swaping these pair won't result in any change in target measures since they're below the cut-off point
					if(j > cutoff && k > cutoff) {
						break;
					}//end-if
					DataPoint p2 = rl.get(k);
					int mk = idx[k];
					//System.out.println("LambadaMART j="+j+" k="+k+" mk="+mk+" j_lbl="+p1.getLabel()+" k_lbl="+p2.getLabel());
					if(p1.getLabel() > p2.getLabel())
					{
						double deltaNDCG = Math.abs(changes[j][k]); //yyl: change neg scores to pos
						if(deltaNDCG > 0)
						{
							...
							weights[mj] += delta;
							weights[mk] += delta;
							//System.out.println("LambadaMART deltaNDCG="+deltaNDCG+" modelScore_mj="+modelScores[mj]+" modelScores_mk="+modelScores[mk]);
							//System.out.println("LambadaMART rho="+rho+" lambda="+lambda+" delta="+delta);
							//System.out.println("LambadaMART pRep["+mj+"]="+pseudoResponses[mj]+" pRep_mk["+mk+"]="+pseudoResponses[mk]);
						}//end-if
					}//end-if p1_y > p2_y
				}//end-for each k
			}//end-for each j
	...
	}//end-function: computePseudoResp
	...
	protected int[] sortSamplesByFeature(DataPoint[] samples, int fid)
	{
		...
		//System.out.println("LambdaMART sortSamplesByFeature: samples.length="+samples.length);  //yyl: samples.length
		for(int i=0;i<samples.length;i++)
			score[i] = samples[i].getFeatureValue(fid);
		int[] idx = MergeSorter.sort(score, true); 

		//for(int i=0;i<idx.length;i++)//yyl: print idx & val
		//	System.out.println("LambdaMART sortSamplesByFeature: sorted idx="+idx[i]+" value="+score[idx[i]]);  //yyl: print idx&val
	...
	{
		//System.out.println("LambdaMART sortSampleByFea:" + martSamples.length); //yyl

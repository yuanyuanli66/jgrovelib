...
public class NDCGScorer extends DCGScorer {
	...
	public double[][] swapChange(RankList rl)
	{
		...
		//System.out.println("NDCG: size="+size); //yyl: min(topK, #obsQid)

		//compute the ideal ndcg
		int[] rel = getRelevanceLabels(rl);
		//for (int i=0; i<rel.length; i++)
		//	System.out.println("NDCG: rel="+rel[i]); //yyl: rel score (output y)
		
		double ideal = 0;
		Double d = idealGains.get(rl.getID());
		//System.out.println("NDCG: idealGain="+d); //yyl: ideal gain init was null
		...
		//yyl: compute changes for doc_ij=doc_ji NOTE:i!=j, doc in orig sample order, idx starts from 0
		for(int i=0;i<size;i++) {
			for(int j=i+1;j<rl.size();j++) {
				if(ideal > 0) {
					changes[j][i] = changes[i][j] = (discount(i) - discount(j)) * (gain(rel[i]) - gain(rel[j])) / ideal;
					//System.out.println("NDCG swapChange discount("+i+")="+discount(i)+" discount("+j+")="+discount(j)+" gain(rel["+i+"])="+gain(rel[i])+" gain(rel["+j+"])="+gain(rel[j])); //yyl
					//System.out.println("NDCG swapChange changes["+j+"]["+i+"]="+changes[j][i]+" ideal="+ideal+" DCG="+changes[j][i]*ideal); //yyl
				}//end-if
			}//end-for j
		}//end-for i
		return changes;
	...
	{
		//yyl: sort obs. based on rel score (y/ranks)
		int[] idx = Sorter.sort(rel, false);
		double dcg = 0;
		for(int i=0;i<topK;i++) {
			dcg += gain(rel[idx[i]]) * discount(i);
			
			//System.out.println("NDCG: getIdealDCG dcg="+dcg+" rel[idx]="+rel[idx[i]]+" gain="+gain(rel[idx[i]])+" dis="+discount(i)); //yyl: rel score (output y)
